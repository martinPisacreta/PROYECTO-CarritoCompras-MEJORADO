...
/**
 * add event listener to anchors of articulo to prevent default.
 */
export function preventArticuloDefault() {
    let items = document.querySelectorAll( ".articulo-nav a, .articulo-size a" );
    for ( let i = 0; i < items.length; i++ ) {
        items[ i ].addEventListener( "click", preventDefaultHandler, true);
    }
}

export function removePreventArticuloDefault() {
    let items = document.querySelectorAll( ".articulo-nav a, .articulo-size a" );
...
/**
 * utils to handle hover event in articulo
 */
export const hoverIntent = function () {
    let items = document.querySelectorAll( '.articulo-3' );
    ...
    for ( let i = 0; i < items.length; i++ ) {
        ...
        function mouseOverHandler() {
            let animDiff = item.offsetHeight - ( item.querySelector( ".articulo-body" ).offsetHeight + item.querySelector( ".articulo-media" ).offsetHeight );
            let animDistance = item.querySelector( ".articulo-footer" ).offsetHeight - animDiff;
            item.querySelector( ".articulo-footer" ).setAttribute( "style", "visibility: visible; transform: translateY(0);" );
            item.querySelector( ".articulo-body" ).setAttribute( "style", "transform: translateY(" + -animDistance + "px)" );
        }

        function mouseLeaveHandler() {
            item.querySelector( ".articulo-footer" ).setAttribute( "style", "visibility: hidden; transform: translateY(100%);" );
            item.querySelector( ".articulo-body" ).setAttribute( "style", "transform: translateY(0)" );
        }
    }
}
...
/**
 * utils to make quanity input in articulo page
 */
...
/**
 * utils to set Single Articulo Gallery
 */
export const articuloGallery = () => {
    let items = document.querySelectorAll( ".articulo-gallery-item" );

    for ( let i = 0; i < items.length; i++ ) {
        let item = items[ i ];

        item.addEventListener( "click", articuloGalleryClickHandler );

        function articuloGalleryClickHandler( e ) {
            for ( let i = 0; i < items.length; i++ )
                items[ i ].classList.remove( "active" );

            item.classList.add( "active" );

            let images = document.querySelectorAll( ".articulo-main-image img" )

            images[ 0 ].setAttribute( "src", item.getAttribute( "data-image" ) );
            images[ 1 ].setAttribute( "src", item.getAttribute( "data-zoom-image" ) );
            document.querySelector( ".articulo-main-image" ).setAttribute( "index", i );
            e.preventDefault();
        }
    }
}